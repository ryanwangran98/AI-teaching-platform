...
#### 5.1.2 测试账号
- 管理员用户: admin1@example.com / password123
- 教师用户: teacher1@example.com / password123
- 学生用户: student1@example.com / password123
...
### 5.2 课程学习流程（学生）
1. 学生登录后进入学生仪表盘
2. 点击左侧菜单"我的课程"，进入已加入课程列表页面
3. 选择一门课程，点击"进入学习"按钮，进入课程详情页面
...
### 5.2.1 加入课程流程（学生）
1. 学生登录后进入学生仪表盘
2. 点击左侧菜单"加入课程"，进入课程浏览页面
3. 浏览所有教师已发布的课程，已加入的课程会有"已加入"标记
4. 选择未加入的课程，点击"加入学习"按钮加入课程
5. 已加入的课程无法再次加入，点击"进入学习"按钮可直接进入课程学习
6. 成功加入课程后，课程会显示在"我的课程"列表中
...
- **防重复提交机制**：确保学生只能提交一次作业

#### 5.5.3 数据提交流程
- **前端提交**：通过FormData格式提交JSON序列化的答案数据或直接提交JSON格式的答案数组
- **后端处理**：
  - 验证作业是否存在且已发布
  - 验证用户是否已提交过该作业
  - 解析JSON答案，验证题目有效性
  - 根据不同题型自动计算得分
- **数据存储**：创建提交记录，保存提交时间、得分等信息
- **结果反馈**：返回提交成功状态和得分信息
...
- 实现了基于角色的权限控制，学生只能查看和提交自己的作业，教师可以查看和管理自己课程的所有作业提交
